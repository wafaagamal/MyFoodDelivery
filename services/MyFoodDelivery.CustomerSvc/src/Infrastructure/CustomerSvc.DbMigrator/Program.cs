...
try
{
    Log.Information("Starting CustomerSvc.DbMigrator...");

    var builder = Host.CreateApplicationBuilder(args);
    
    builder.Services.AddDbContext<CustomerSvcDbContext>(options =>
    {
        var connectionString = builder.Configuration.GetConnectionString("CustomerSvcWrite");
...
{
    Log.Fatal(ex, "CustomerSvc.DbMigrator terminated unexpectedly!");
    throw;
}
finally
{
...
public class DbMigratorHostedService : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            var dbContext = scope.ServiceProvider.GetRequiredService<CustomerSvcDbContext>();
